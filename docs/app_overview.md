...
- **Sub-features**:
  ...
  - Optional **consumable recipe** (house chemicals to record on Create Order)
- **Key Models**: `Service`, `ServiceCategory`, `ServicePriceTier`, `ServiceConsumableRecipe`, `CartServiceItem`, `SaleServiceItem`
...
- Sales summary KPI cards (including Needs Attention for incomplete order details, Add-ons Sold, and **Consumables used** when the org flag is on); collapsing Today's Summary hides it for the rest of the local day on this device. A labeled Refresh control reloads all dashboard data (1s minimum) and shows a brief "Pulled new data complete" toast
...
- Org/branch URL scope (`/{orgSlug}/{branchSlug}/…`) with `.goScoped` / `.pushScoped`
- Route files organized by domain
- Shell-based routing with nested subroutes
- Public `/history` and version-lock routes stay outside the scope prefix
...
### ~30 collections across 9 domains
...
| Collection | Description |
|------------|-------------|
| `products` | Products/inventory items (including house consumables) |
...
| `serviceConsumableRecipes` | Default house-chemical usage lines for a service |
...
| `saleItems` | Product items in a transaction (add-on revenue) |
| `saleServiceItems` | Service items in a transaction |
| `saleConsumableUsages` | House-chemical quantities recorded on an order |
...
| Collection | Description |
|------------|-------------|
| `featureFlags` | Per-organization workflow toggles (Organizations → Features) |
...
- **Reports**: Sales, payroll, and consumable-usage reports by period
