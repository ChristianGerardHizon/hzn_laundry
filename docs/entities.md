...
| Product | Product | `products` | Products/inventory items (including house consumables) |
...
| Service | ServiceConsumableRecipe | `serviceConsumableRecipes` | Default house-chemical usage for a service |
...
| Sales | SaleConsumableUsage | `saleConsumableUsages` | House-chemical quantity recorded on a sale |
...
| `slug` | String | Yes | URL-safe slug unique within the organization (`all` reserved) |
...
| `isConsumable` | bool | Yes | House chemical/supply (default false) |
| `countsTowardMaterialCost` | bool | Yes | Add-on qty also counts as usage/cost (default false) |
| `usageMin` | num | Yes | Minimum usage on an order, never negative (default 0) |
| `usageMax` | num | No | Maximum usage; null = no cap |
| `usageStep` | num | No | Stepper increment; null/0 treated as 1 |
| `defaultUsage` | num | No | Default when a recipe prefills this product |
| `isDeleted` | bool | Yes | Soft delete flag |
| `created` | DateTime | No | Creation timestamp |
| `updated` | DateTime | No | Last update timestamp |
...
Default house-chemical usage for a service. Copied onto the sale at Create Order; the order then owns the numbers.
...
| `serviceId` | String (FK) | Yes | FK to Service |
| `productId` | String (FK) | Yes | FK to Product |
| `defaultQuantity` | num | Yes | Prefill amount (default 0) |
| `prefill` | bool | Yes | If false, staff must enter a value before creating the order |
| `product` | Product | No | Expanded product |
| `created` | DateTime | No | Creation timestamp |
| `updated` | DateTime | No | Last update timestamp |

**Collection:** `serviceConsumableRecipes`

**Relationships:** `serviceId` -> Service; `productId` -> Product. Unique `(service, product)`.
...
House-chemical quantity recorded on a sale. Separate from `saleItems` (add-on revenue). Not printed on customer receipts.
...
**Collection:** `saleConsumableUsages`
...
Per-organization workflow toggle (email updates, require machine/pack/storage, consumable usage). Edited on Organizations → Features (`/organizations/:id`). Unique `(organization, key)`.
...
**Total PocketBase collections (excluding SQL views):** 39 named constants in `pocketbase_collections.dart`, plus `orderStatusHistory` (referenced by literal string, not centralized as a constant) ≈ 40
