...
Authenticated app routes live under a hand-written org/branch prefix
`/{orgSlug}/{branchSlug}/…`. Feature `path:` constants stay flat
(`/dashboard`, `/products`, …). Navigate with `.goScoped(context)` /
`.pushScoped(context)` under that shell (see
`lib/src/core/routing/org_scoped_navigation.dart`). Auth, version-lock, and
public `/history/:token` stay outside the prefix.

### Route Tree (per `lib/src/core/routing/routes/*.routes.dart`)

```
/{orgSlug}/{branchSlug}/dashboard   -> Dashboard
│
