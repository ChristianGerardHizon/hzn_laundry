...
class PrintBluetoothThermalPlugin: FlutterPlugin, MethodCallHandler {
  ...
  /// Opens an RFCOMM socket directly on [channel], bypassing SDP lookup via
  /// the hidden `createRfcommSocket(int)` API. Many cheap ESC/POS thermal
  /// printers have broken/incomplete SDP records, which makes the standard
  /// `createRfcommSocketToServiceRecord` connect fail with
  /// "read failed, socket might closed or timeout, read ret: -1" even
  /// though the device is on, paired, and in range. Channel 1 is the
  /// conventional SPP channel these printers listen on.
  private fun createRfcommSocketByChannel(
    device: BluetoothDevice,
    channel: Int
  ) = device.javaClass
    .getMethod("createRfcommSocket", Int::class.javaPrimitiveType)
    .invoke(device, channel) as android.bluetooth.BluetoothSocket
    ...
    return withContext(Dispatchers.IO) {
      ...
      if (bluetoothAdapter != null && bluetoothAdapter.isEnabled) {
        ...
        try {
          ...
          Log.d(TAG, "connect (SDP): ${e.message} code ${e.hashCode()} — falling back to channel 1")
          // SDP-based lookup failed (common on printers with broken SDP
          // records). Retry by opening RFCOMM channel 1 directly.
          try {
            val fallbackSocket = createRfcommSocketByChannel(bluetoothDevice, 1)
            ...
            if (fallbackSocket.isConnected) {
              ...
              state = false
              Log.d(TAG, "Desconectado (channel 1 fallback): ")
            }
          } catch (fallbackError: Exception) {
            state = false
            Log.d(TAG, "connect (channel 1 fallback): ${fallbackError.message} code ${fallbackError.hashCode()}")
            outputStream?.close()
          }
        }
      }else{
