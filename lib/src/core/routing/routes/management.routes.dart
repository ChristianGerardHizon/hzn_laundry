...
class _MachineDetailWrapper extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return _ManagementMachineDetailPage(machine: machine);
  }
}

/// Machine detail page for organization section.
class _ManagementMachineDetailPage extends ConsumerWidget {
  const _ManagementMachineDetailPage({required this.machine});
                  ...
                  children: [
                    ...
                    if (machine.created != null)
                      _detailRow(theme,
                          icon: Icons.calendar_today,
                          label: 'Created',
                          value: dateFormat.format(machine.created!.toLocal())),
                    if (machine.updated != null)
                      _detailRow(theme,
                          icon: Icons.update,
                          label: 'Last Updated',
                          value: dateFormat.format(machine.updated!.toLocal())),
                  ],
                ),
              ),
            ),
  ...
  Future<void> _handleDelete(BuildContext context, WidgetRef ref) async {
    ...
    if (context.mounted) {
      if (success) {
        showSuccessSnackBar(context, message: 'Machine deleted successfully');
        context.pop();
      } else {
        showErrorSnackBar(context,
